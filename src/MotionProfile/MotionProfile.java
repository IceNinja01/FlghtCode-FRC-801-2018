...
public class MotionProfile {
	 ...
	 */
	public static double[][] OneDimenisonMotion(double distance, double maxVel, double accel, double heading, double endingVel) {
		...
		for(int i = 1; i < segments; i++)
		{
			...
			path[i][2] = dt*1000; //time
			path[i][3] = heading;  //heading
		...
		path[segments-1][1] = 0;
		path[segments-1][2] = dt*1000;
		path[segments-1][3] = heading;
	...
	public static double[][] XYCoord(){
		 ...
		 */
		double[][] path= OneDimenisonMotion(.5, .5, .5 , 0,0);
	...
	public static double[][] rampUp(double[] wayPoint){
		/* distance is units ft
		 * MaxVelocity is ft/sec
		 * accel is ft/sec^2
