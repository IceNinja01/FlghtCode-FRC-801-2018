...
public class SwerveDrive implements MotorSafety
{
	...
	public SwerveDrive(final Team801TalonSRX FrontRightDriveMotor, final Team801TalonSRX FrontLeftDriveMotor,
			final Team801TalonSRX BackLeftDriveMotor, final Team801TalonSRX BackRightDriveMotor,
			final Team801TalonSRX FrontRightTurnMotor, final Team801TalonSRX FrontLeftTurnMotor,
			final Team801TalonSRX rearLeftTurnMotor, final Team801TalonSRX rearRightTurnMotor, int avgSize)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			turnMotors[i].configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Absolute, 0,
			...
			// turnMotors[i].configAllowableClosedloopError(0, deadBand,
			// Constants.kTimeoutMs);//1 degree error=4096/360, expressed in native units
			//
			// set coast mode
			...
			/*
			 * the sensor and motor must be in-phase. This means that the sensor position
			 * must move in a positive direction as the motor controller drives positive
			 * motor output. To test this, first drive the motor manually (using gamepad
			 * axis for example). Watch the sensor position either in the roboRIO Web-based
			 * Configuration Self-Test, or by calling GetSelectedSensorPosition() and
			 * printing it to console. If the Sensor Position moves in a negative direction
			 * while Talon SRX motor output is positive (blinking green), then use the
			 * setSensorPhase() routine/VI to multiply the sensor position by (- 1). Then
			 * retest to confirm Sensor Position moves in a positive direction with positive
			 * motor drive.
			 **/
			turnMotors[i].setSensorPhase(false);

			driveMotors[i].configSelectedFeedbackSensor(FeedbackDevice.CTRE_MagEncoder_Relative, 0,
					Constants.kTimeoutMs);
	 ...
	 *            produce. [-1.0..1.0]
	 * @param rotation
	 *            The rate of rotation for the robot that is completely independent
	...
	public void drive(double AxisX, double AxisY, double rotation, double gyroAngle)
	{
		...
		FWD = temp;
		// Perform the following calculations for each new set of FWD, STR, and RCW
		// commands:
		A = STR - RCW * (L / R);
		B = STR + RCW * (L / R);
		C = FWD - RCW * (W / R);
		...
		for (int i = 0; i < 4; i++)
		{
			...
			if (Math.abs(angleJoyStickDiff[i]) > 90)
			{ // new angle is greater than a 90degree turn, so find shortest path
				...
				if (wheelAngles[i] < 0)
				{ // wrap to new angle between 0-360
					wheelAngles[i] += 360.0;
				}
