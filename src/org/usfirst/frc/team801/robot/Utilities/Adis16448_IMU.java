...
public class Adis16448_IMU extends GyroBase implements Gyro, PIDSource, Sendable
{
	 ...
	 */
	/*
	 * @Override public void updateTable() { ITable table = getTable(); if (table !=
	 * null) { table.putNumber("Value", getAngle()); table.putNumber("Pitch",
	 * getPitch()); table.putNumber("Roll", getRoll()); table.putNumber("Yaw",
	 * getYaw()); table.putNumber("AccelX", getAccelX()); table.putNumber("AccelY",
	 * getAccelY()); table.putNumber("AccelZ", getAccelZ());
	 * table.putNumber("AngleX", getAngleX()); table.putNumber("AngleY",
	 * getAngleY()); table.putNumber("AngleZ", getAngleZ()); } }
	 */
}
